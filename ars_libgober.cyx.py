...
class ArsSampler(object):
    ...
    Args:
        initial_knots (np.array): Locations in the domain to initialize.
            Include at least three knots. At least one knot 
            should be on each side of the mode. 
            Nothing in the module will guarantee that it's been initialized properly. 

        h (np.vectorized function): log of the pdf of the distribution.
            The algorithm does not the pdf to be normalized.
            This implies that any specification +/- some consant K is fine.
            
        hprime (np.vectorized function) : derivative of the log pdf
             
            
        xlb (float) : Lower bound of domain of the pdf. Defaults to -np.inf
             
            
        xlb (float) : Lower bound of domain of the pdf. Defaults to np.inf
         
        
        max_knots (int) : The maximum number of knots in envelope. Default = -50
            Essentially a tuning parameter. 
            
            This is one less than the number of piecewise linear functions
            that make up the envelope. 
            
            More pieces => it takes longer to draw from the envelope
            Tigher envelope =>  draws are less likely to be rejected
       
            The sweetspot depends on your distribution
    """
...
#%% Create the example showing that the sampler works if not called as module        
if __name__ == '__main__':       
    np.random.seed(1)
    initial_knots = [-1,0,1]
